...
ctx.font = '20 consolas';
ctx.fillStyle = 'white';
function main(){
ctx.save();
  ctx.fillStyle = '#00bae9';
  ctx.beginPath();
    ctx.rect(0, TOPHEIGHT, canvas.width, (canvas.height - TOPHEIGHT) * (values.pitch * 4 <= -MAXSIZE/2 ? 0 : 1));
  ctx.closePath();
  ctx.fill();
  //CENTER
  ...
  for(i = 0; i < 12; i++){
    ...
    if(xCache != 25){
      ...
      ctx.fillText(textCache, -xCache - (textCache.length * FONTWIDTH), yCache + FONTHEIGHT/4);
    }
  ...
  ctx.fillText(textCache, -FONTWIDTH * textCache.length - 14, FONTHEIGHT/4 + 2);
  ctx.restore();
  ctx.save();
  ctx.beginPath();
  ...
  ctx.stroke();
  ctx.fillStyle = 'white';
  ctx.fillText(Math.round(values.altitude), 12 + 3 + FONTWIDTH * 5 - FONTWIDTH * Math.round(values.altitude).toString().length, FONTHEIGHT/4 + 2)
  ctx.fillStyle = 'black';
  ctx.beginPath();
  ...
  ctx.fillText(Math.round(values.altitude).toString().split('')[Math.round(values.altitude).toString().length - 3] || '', 12 + 3 + FONTWIDTH * 2.1, FONTHEIGHT*0.75/4 + 2)
